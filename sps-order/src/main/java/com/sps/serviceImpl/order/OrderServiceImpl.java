...
public class OrderServiceImpl implements OrderService {
	 ...
	 * @param name 店主名称
	 * @param orderid 订单编号
	 * @param startTime 订单申请开始日期
	 * @param endTime 订单申请结束日期
	 * @param flag 订单状态
	 * @return
	 */
 ...
 */
	...
	//批量更新，需要加入事务
/*	@Transactional
	public Integer updatePriceBatch(List<OrderGoods> list){
		Integer result=orderGoodsMapper.updatePriceBatch(list);
	...
	/**
	 * //单个修改价格
	 * @param orderGoods 传入的最新的OrderGoods对象
	 * @return
	 */
	
	/*public int updatePrice(OrderGoods orderGoods) {
		//获取sku编号
		String sku=orderGoods.getSku();
		
		//根据sku编号值会查出来的只包含一个OrderGoods的List
		List<OrderGoods> myOrderGood=orderGoodsMapper.selectOrderGoods(null, sku);
		
		//将根据sku查出来的一个orderGood的List转换成OrderGoods对象，此orderGood和最新的orderGoods的sku值是一样的，其他的可能不一样
		OrderGoods orderGood=myOrderGood.get(0);
		...
		//之前的价格
		BigDecimal preprice=orderGood.getPrice();
		//当前要设置的价格
		BigDecimal price=orderGoods.getPrice();
		
		orderGoodss.setAmount(amount);
		orderGoodss.setModifytime(new Date());
		//设置修改前的订货量,将未修改前的订货量设置到新对象中
		orderGoodss.setPreAmount(orderGood.getAmount());
		orderGoodss.setPrice(price);
		//最新的价格和订货量对应的总金额
		Double summation=amount.doubleValue()*price.doubleValue();
		orderGoodss.setSummation(summation);
	 ...
	 * @param orderGoodses 前台页面传入的订单商品的列表
	 * @return
	 */
//	public HashMap<String, Object> updatePriceBatch(List<OrderGoods> orderGoodses) {
//		
//		HashMap<String, Object> hashMap = new HashMap<String, Object>();
...
//		hashMap.put("data", orderGoodsListNew.size() > 0 ? orderGoodsListNew : null);
		...
		// 根据orderid查出商品列表
		List<OrderGoods> listOrderGoods=new ArrayList<OrderGoods>();
		List<OrderGoods> orderGoods = this.getOrderGoods(orderid,null);
		for (OrderGoods orderGood : orderGoods) {
			OrderGoods newOrderGoods = new OrderGoods();
			Integer amount=orderGood.getAmount();//订货量	
			newOrderGoods.setPreAmount(orderGood.getAmount());
			newOrderGoods.setAmount(amount);//这里是新设定的订货量
			Double summation=amount.doubleValue()*price.doubleValue();//合计
			newOrderGoods.setPrePrice(orderGood.getPrice());//将之前的价格设置为从数据库中查询出的价格，也就是未更新的价格
			newOrderGoods.setPrice(price);//将价格修改为传入的价格
		...
		List<OrderGoods> orderGoodses= orderGoodsMapper.updatePriceBatch(listOrderGoods);
		// 转为pageinfo
...
public HashMap<String, Object> updatePriceBatch(List<OrderGoods> goods) {
	...
	/**
	 * 这里的flag 状态分为三个，2为已拒绝，6修改为待发货，拒绝的时候必须有message的说明原因,对于flag状态可以最后进行统一设定，这里的状态并不作为最终状态码
	 */
	@Override
...
/*	@Override
	public HashMap<String,Object> selectByExpressPrimaryKey(Integer id) {
		HashMap<String, Object> hashMap = new HashMap<String, Object>();
		List<Express> list=new ArrayList<Express>();
		Express express=expressMapper.selectByExpressPrimaryKey(id);
		list.add(express);
		hashMap.put("code", 0);
		hashMap.put("msg", "获取成功");
...
//	@Override
//	public HashMap<String, Object> updatePriceBatch(Integer page, Integer limit, String orderid, BigDecimal price) {
//		// TODO Auto-generated method stub
//		return null;
//	}

	/*@Override
	public HashMap<String, Object> selectBySku(Integer page, Integer limit, String sku) {
		HashMap<String, Object> hashMap = new HashMap<String, Object>();
