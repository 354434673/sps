...
public class StringUtil {
	 ...
	 * @return
	 */
	public static boolean isLegal(String jsonString) {
		try {
			if (StringUtil.isEmpty(jsonString)) {
				return false;
			}
			JSONObject param = JSONObject.parseObject(jsonString, JSONObject.class);
			Class<?> utilClass = StringUtil.class;
			for (String key : param.keySet()) {
				if (param.get(key) instanceof java.util.List) {
					isLegal(JSONObject.toJSONString(param.get(key)));
				} else {
					String me = key.substring(0, 1).toUpperCase() + key.substring(1);
					Method m1 = utilClass.getMethod("is" + me, Object.class);
					Object userInfo = utilClass.newInstance();
					if (!(Boolean) m1.invoke(userInfo, param.get(key))) {
						return false;
					}
				}
			}
