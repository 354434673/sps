...
public class UserServiceImpl implements UserService {
	 ...
	 * 上午11:59:27 @return: SpsUser @throws
	 */
	...
	public HashMap<String, Object> userList(Integer page, Integer limit, String username, String name, String phone, String userSupplierNum,
		...
		Criteria createCriteria = example.createCriteria();
		createCriteria.andUserStateNotEqualTo(1).andUserMarkEqualTo(mark);
		// 如果查询条件不为空,则进入模糊查询
	...
	public HashMap<String, Object> updateUserState(String userName, Integer state) {
		...
		try {
			...
			SpsLogExample logExample = new SpsLogExample();
			com.sps.entity.log.SpsLogExample.Criteria createCriteria = logExample.createCriteria();
			createCriteria.andLogNameEqualTo(userName);
			createCriteria.andLogTypeEqualTo("userLogin");

			List<SpsLog> list = logMapper.selectByExample(logExample);
			if (list.size() > 0) {
				map.put("msg", "用户不可删除");
				map.put("icon", "2");
				map.put("state", FinalData.STATE_ERROR);
